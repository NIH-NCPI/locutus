...
class TableInstitutionAccess(Resource):
    """S1: any editor (not just the owner) can add or remove an
    institution's access to this table."""
    ...
    def put(self, id: str, institution_id: str):
        ...
        resource = set_institution_access("Table", id, institution_id, role)
        return json.loads(json_util.dumps(resource)), 200, default_headers

    @require_write_access("Table", "id")
    def delete(self, id: str, institution_id: str):
        if not remove_institution_access("Table", id, institution_id):
            return (
                {"message": f"{institution_id} does not have access to this table"},
                404,
                default_headers,
            )

        # require_write_access already confirmed this id exists.
        t = mTable.get(id, return_instance=False)
        assert t is not None
        return json.loads(json_util.dumps(t)), 200, default_headers


class TableVisibility(Resource):
    """S2: owner-only toggle of this table's visibility."""
    ...
    def put(self, id: str):
        ...
        resource = mTable.get(id, return_instance=False)
        ...
        updated = set_visibility("Table", id, Visibility(visibility))
