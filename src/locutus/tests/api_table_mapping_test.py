...
def test_table_mapping_get_missing_table_raises(client):
    # Documents current behavior: TableMapping.get does not check for a None
    # Table before calling .terminology.dereference() on it.
    with pytest.raises(AttributeError):
        client.get("/api/Table/not-there/mapping/string_var")
...
def test_table_mappings_get_missing_table_raises(client):
    # Documents current behavior: TableMappings.get_mappings does not check
    # for a None Table before calling .terminology.dereference() on it.
    with pytest.raises(AttributeError):
        client.get("/api/Table/not-there/mapping")
