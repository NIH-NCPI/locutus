...
def test_get_preference_terminology(sample_terminology):
    ...
    assert sample_terminology.get_preference()["self"]["api_preference"] == {
        "ols": ["HP", "MONDO"]
    }
    ...
    assert sample_terminology.get_preference("C99")["self"]["api_preference"] == {
        "ols": ["HP", "MONDO"]
    }
...
def test_get_preferred_terminology(sample_terminology, all_terminologies):
    ...
    sample_terminology.replace_preferred_terminology(
        "unit-test", [{"preferred_terminology": term}]
    )
    assert sample_terminology.get_preferred_terminology()["references"] == [
        {"reference": f"Terminology/{term}"}
    ]


def test_replace_preferred_terminology(
    sample_terminology, ftd_concept_relationships, all_terminologies
):
    term1 = all_terminologies[1].id
    term2 = all_terminologies[0].id
    assert sample_terminology.get_preferred_terminology()["references"] == []
    sample_terminology.replace_preferred_terminology(
        "unit-test", [{"preferred_terminology": term1}]
    )
    assert sample_terminology.get_preferred_terminology()["references"] == [
        {"reference": f"Terminology/{term1}"}
    ]
    sample_terminology.replace_preferred_terminology(
        "unit-test", [{"preferred_terminology": term2}]
    )
    assert sample_terminology.get_preferred_terminology()["references"] == [
        {"reference": f"Terminology/{term2}"}
    ]
