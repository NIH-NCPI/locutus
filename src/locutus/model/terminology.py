...
class Terminology(Serializable):
    ...
    def delete(self, hard_delete=True):
        ...
        for prov in locutus.model.provenance.Provenance.find(
        ...
        if type(code) is str:
            ...
            if coding is not None:
                ...
                if type(coding) is list and len(coding) != 0:
                    ...
                    raise locutus.model.exceptions.InvalidValueError(
                        code, "too many codes already exist in the database"
                    )
            ...
            if coding is not None:
                if coding.valid is False:
                    ...
                    raise locutus.model.exceptions.CodeAlreadyPresent(
                        code, self.id, coding
                    )
    ...
    def get_provenance(self, code=None):
        ...
        if code is None or code == "self":
            ...
            for prv in locutus.model.provenance.Provenance.terminology_provenance(
                terminology_id=self.id
            ):
            ...
            for prv in locutus.model.provenance.Provenance.mapping_provenance(
                terminology_id=self.id, target_coding=code
            ):
                if prv is not None:
                    if type(prv["timestamp"]) is not str:
                        prv["timestamp"] = prv["timestamp"].strftime(
    ...
    def add_provenance(
        ...
        if target is None or target == "self":
            ...
            if target.count("|") > 1:
                print(
                    f"Warning: Invalid target format '{target}'. Skipping provenance addition."
                )
                return
    ...
    def remove_pref(self, code=None):
        if code is None or code == "self":
            if len(self.api_preferences) > 0:
                message = (
                    f"Successfully deleted preferences for Terminology '{self.name}'."
                )
            else:
    ...
    def remove_preferred_terminology(self, editor=None):
        if len(self.preferred_terminologies) > 0:
            ...
            self.add_provenance(
                ...
                target="self",
                old_value=",".join(
                    [x["reference"] for x in self.preferred_terminologies]
                ),
                editor=editor,
            )
        else:
...
class MappingUserInputModel:
    def generate_mapping_user_input(id, code, mapped_code, user_id):
        ...
        user_vote = ""
        if votes != []:
            vote_count = MappingUserInputModel.get_mapping_votes_counts(
                votes["mapping_votes"]
            )
