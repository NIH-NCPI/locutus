...
class UserInput:
    ...
    @classmethod
    def get_user_input(
        cls, resource_type, collection_type, id, code, mapped_code, type
    ):
        """
    ...
    def delete_user_conversations(
        cls, resource_type, collection_type, id, code, mapped_code, hard_delete=False
    ):
        user_input = MappingConversation.get(
            terminology_id=id,
            source_code=code,
            mapped_code=mapped_code,
    ...
    def exists_in_db(self, return_instance=True):

        item = self.__class__.get(
    ...
    def get(
        ...
        if not isinstance(terminology_id, str) or not terminology_id.strip():
            raise ValueError(
                "Terminology ID is required to get a coding without an _id."
            )
    ...
    @classmethod
    def create_or_replace_user_input(
        cls,
        resource_type,
        collection_type,
        id,
        ...
        try:
            ...
            user_input_instance = input_class(
                terminology_id=id, source_code=code, mapped_code=mapped_code
            )

            # Build and format user_input as necessary
            user_input_instance.add_input(
                input_value, editor=editor, timestamp=timestamp
            )
    ...
    def update_or_append_input(cls, existing_data, user_id, new_record, return_format):
        ...
        if return_format is dict:
            ...
            if isinstance(existing_data, list):
                ...
                raise ValueError(
                    "The existing data should be of return_format type 'list'."
                )
...
class MappingConversation(Simple, UserInput):
    ...
    def __init__(
        ...
        UserInput.__init__(
            self, return_format=list, input_type="note", update_policy="append"
        )
    ...
    def build_user_input(self, note, editor=None, timestamp=None):
        ...
        return {"user_id": user_id, "note": note, "date": date}

    def validate_input(self, user_input):
        note = user_input.get("note")
        if not note or len(note) > USER_INPUT_CHAR_LIMIT:
            raise ValueError(
                f"'note' must be provided and cannot exceed {USER_INPUT_CHAR_LIMIT} characters."
            )
        else:
            pass
    ...
    class _Schema(Schema):
        ...
        mapping_conversations = fields.List(
    ...
    def to_dict(self):
        ...
        return {
            "mapping_conversations": [
                {"user_id": conv["user_id"], "date": conv["date"], "note": conv["note"]}
...
class MappingVote(Simple, UserInput):
    ...
    def build_user_input(self, user_input, editor=None, timestamp=None):
        ...
        if type(vote) is dict:
            vote = vote["vote"]
        return {"user_id": user_id, "vote": vote, "date": date}
    ...
    def _build_mapping_votes(self, mapping_votes):
        ...
        return {
            vote["user_id"]: {"vote": vote["vote"], "date": vote["date"]}
            for vote in mapping_votes
        }
