...
class GlobalID(Simple):
    ...
    def __init__(
        ...
        if resource_type not in locutus.model.resource_types:
            ...
            raise ValueError(
                f"resource_type, '{resource_type} is not a valid resource type. Available options include: {resource_list}"
            )
        ...
        if self.id is None:
            self.id = (
                f"{locutus.model.resource_types[resource_type]._id_prefix}-{generate()}"
            )
            self.save()
    ...
    def save(self):
        ...
        if self._id is None:
            id_matches = self.__class__.find(
                resource_type=self.resource_type, key=self.key
            )
    ...
    def find(cls, resource_type, key=None, domain="", return_instance=True):
        params = {"resource_type": resource_type, "domain": domain}
